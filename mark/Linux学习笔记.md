...
- 通过进程号杀死进程
  - kill [选项]进程号
- 通过进程名称杀死进程,也支持通配符,这个在系统因负载过大而变得慢时很有用
...
2) 选项说明
- 指定top命令每隔几秒更新,默认时3喵在top命令的交互模式当中可以执行的命令
  - -d 秒数
- 使top不显示任何限制或者僵死进程
  - -i
- 通过指定监控ID来仅仅监控某个进程的状态
...
3) 参数说明`crontab -e`
   ...
   - *****执行的任务
    ...
    - ,(逗号)
      - 代表不连续的时间,比如"0 8 ,12,16 * * * 命令"就代表在每天的 8点0分,12点0分,16点0分都执行一次命令
    - -(横杠)
      - 代表连续的时间范围,比如(0 5 * * 1-6 命令),代表在周一到周六的凌晨5点0分执行命令
    - */n
      - 代表每隔多久执行一次,比如"*/10 * * * * "命令,代表每个10分钟就执行一次命令
...
RPM(RedHat Package Manager),RedHat软件包管理工具,类似于windows里面的setup.exe,是Linux这系列操作系统里面的打包工具,它虽然是RedHat的标志,但理念是通用的
